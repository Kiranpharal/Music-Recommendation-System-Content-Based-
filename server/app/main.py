...
# ⬅️ NEW IMPORTANT IMPORT
from .recommender import search_titles, get_recommendations, ensure_dataset_downloaded
...
async def startup():
    ...
    await ensure_dataset_downloaded()     # ⬅️ IMPORTANT NEW LINE
...
def login(req: LoginRequest, db: Session = Depends(get_db)):
    user = (
        ...
        .filter(
            ...
            (models.User.username == req.identifier)
        )
        .first()
    )
...
def admin_create(req: AdminCreateRequest,
...
def update_username(req: UpdateUsernameRequest,
                    user=Depends(get_current_user),
                    db: Session = Depends(get_db)):
...
def update_email(req: UpdateEmailRequest,
    ...
    return {
        "message": "Email updated. Verification sent again.",
        "email": req.email
    }
...
def liked_songs_list(user=Depends(get_current_user),
                     db: Session = Depends(get_db)):

    rows = (
        db.query(models.LikedSong)
        .filter(models.LikedSong.user_id == user.id)
        .all()
    )
...
def toggle_like(song: SongIn,
    ...
    entry = models.LikedSong(
        ...
        song_id=song_id,
        song_title=song.name,
        song_artist=s.song_artist if hasattr(song, "song_artist") else song.artists,
        song_preview_url=song.preview,
        song_artwork=song.artwork,
        song_youtube_url=song.youtubeUrl,
        song_cluster_mood=song.cluster_mood,
